...
```
stock-exchange-sim/
├── main.go                 # Main stock exchange program
├── checker/                # Checker program directory
│   └── checker.go          # Checker Struct 
│   └── loadconfig.go       # LoadConfig method
│   └── new-hecker.go       # NewChecker  function
│   └── verify.go           # Verify method
├── engine/                 # Schedulling Implementations 
│   ├── engine.go          # Basic example
│   ├── cabinet         # Cabinet building example
│   └── infinite        # Self-sustaining example
